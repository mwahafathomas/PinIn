...
export default function App() {
  ...
  const [furnitureList, setFurnitureList] = useState<FurnitureItem[]>(() => {
    ...
    return local && local.length > 0 ? local : cleanInitial;
  });
  ...
  // Share Item
  const handleShareItem = (item: FurnitureItem, e?: React.MouseEvent) => {
    if (e) e.stopPropagation();
    navigator.clipboard?.writeText(window.location.href);
    showToast(`Link copied for "${item.title}"`);
  };
  ...
  // Add new listing (Immediately live & synchronized to Supabase with status 'approved')
  const handleAddListing = async (
    ...
    // Make live immediately on screen
    setUserOwnListings((prev) => [newItem, ...prev]);
    setFurnitureList((prev) => [newItem, ...prev.filter((i) => i.id !== newItem.id)]);
    saveLocalListing(newItem);
  ...
  // Share listing from user's account
  const handleShareListing = (item: FurnitureItem) => {
    navigator.clipboard?.writeText(window.location.origin + `/item/${item.id}`);
    showToast(`Shareable link copied for "${item.title}"`);
  };
