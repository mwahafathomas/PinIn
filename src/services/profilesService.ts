...
export function markUserProfileUpdated(userId: string) {
  if (!userId) return;
  try {
    localStorage.setItem(`pinin_profile_completed_${userId}`, 'true');
  } catch {}
}

export function isUserProfileUpdated(userId: string): boolean {
  if (!userId) return false;
  try {
    return localStorage.getItem(`pinin_profile_completed_${userId}`) === 'true';
  } catch {
    return false;
  }
}
...
export async function upsertUserProfile(profile: {
  ...
  try {
    ...
    markUserProfileUpdated(profile.id);
