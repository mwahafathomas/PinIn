...
const isNameFromEmail = (nameStr?: string, emailStr?: string) => {
  ...
  if (clean === '' || clean === 'pininmember' || clean === 'guest' || clean === 'valueduser' || clean === 'user') return true;
...
const checkProfileCompleteness = (u: UserAccount) => {
  const hasAvatar = !!(u.avatar && !isDefaultAvatar(u.avatar));
  const hasFirstName = !!(u.name && u.name.trim() !== '' && !isNameFromEmail(u.name, u.email));
  const hasSurname = !!(u.surname && u.surname.trim() !== '' && u.surname.trim().toLowerCase() !== 'user');
  const hasLocation = !!(
    u.location &&
    u.location.trim() !== '' &&
    u.location !== 'Sandton (Gauteng)' &&
    u.location !== 'Gauteng, South Africa' &&
    u.location !== 'Gauteng'
  );
  return {
    isComplete: hasAvatar && hasFirstName && hasSurname && hasLocation,
    hasAvatar,
    hasFirstName,
    hasSurname,
    hasLocation,
