      ...
      <div className="shrink-0 bg-white/95 backdrop-blur-xs border-b border-gray-200">
        <div className="w-full max-w-md md:max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-2.5 flex items-center justify-between">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 py-1 px-2.5 rounded-full bg-blue-50 hover:bg-blue-100 active:scale-95 text-[#2D8EDE] text-xs font-bold transition-all border border-blue-200/80 shadow-xs cursor-pointer"
          >
            <SlidersHorizontal className="w-3.5 h-3.5" />
            <span>Edit Filters</span>
            {appliedCount > 0 && (
              <span className="w-4 h-4 rounded-full bg-[#2D8EDE] text-white text-[10px] font-black flex items-center justify-center">
                {appliedCount}
              </span>
            )}
          </button>
          ...
          <div className="w-full max-w-md md:max-w-7xl mx-auto px-4 md:px-6 lg:px-8 pb-2 flex items-center gap-1.5 overflow-x-auto no-scrollbar">
            {filters.categories && filters.categories.length > 0 ? (
              filters.categories.map((cat) => (
                <span
                  key={cat}
                  className="bg-blue-100/80 text-[#2D8EDE] font-extrabold text-[11px] px-2.5 py-0.5 rounded-full whitespace-nowrap capitalize shrink-0"
                >
                  {cat.split('/')[0].trim()}
                </span>
              ))
            ) : filters.category && filters.category !== 'all' ? (
              <span className="bg-blue-100/80 text-[#2D8EDE] font-extrabold text-[11px] px-2.5 py-0.5 rounded-full whitespace-nowrap capitalize shrink-0">
                {filters.category}
              </span>
            ) : null}
          </div>
      ...
      <main className="flex-1 w-full max-w-md md:max-w-7xl mx-auto overflow-y-auto px-3 md:px-6 lg:px-8 pt-3 pb-6">
        {items.length > 0 ? (
              ...
              return (
                ...
                >
                  ...
                  <div className="p-2.5 flex-1 flex flex-col justify-between">
                    <div>
                      <h3 className="font-extrabold text-xs text-gray-900 line-clamp-1 group-hover:text-[#2D8EDE] transition-colors">
                        {item.title}
                      </h3>
                    ...
                    <div className="mt-2 pt-2 border-t border-gray-100 flex items-center justify-between text-[11px]">
                      ...
                      {item.distanceText && (
                        <span className="font-bold text-[#2D8EDE] bg-blue-50 px-1.5 py-0.2 rounded text-[10px]">
                          {item.distanceText}
                        </span>
                      )}
                    </div>
          ...
          <div className="my-auto py-16 text-center space-y-3 bg-white rounded-3xl border-2 border-gray-200 p-6">
            <div className="w-14 h-14 rounded-full bg-blue-50 text-[#2D8EDE] flex items-center justify-center mx-auto">
            ...
            <div className="pt-2 flex flex-col sm:flex-row items-center justify-center gap-2">
              {onOpenSell && (
                <button
                  type="button"
                  onClick={onOpenSell}
                  className="w-full sm:w-auto py-2.5 px-5 bg-[#2D8EDE] hover:bg-[#2579BE] active:scale-95 text-white font-extrabold text-xs rounded-xl shadow-xs transition-all cursor-pointer text-center"
                >
