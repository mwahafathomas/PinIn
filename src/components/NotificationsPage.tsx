...
export const NotificationsPage: React.FC<NotificationsPageProps> = ({
  ...
  const handleEnablePush = async () => {
    ...
    setHasPushPermission(result.granted);

    if (result.granted) {
      setPushStatusMessage('🎉 Notifications enabled for this device!');
    } else {
      setPushStatusMessage('💡 Please allow notification permission in your device prompt.');
    }
  ...
  const handleSendTestPush = async () => {
    ...
    const result = await sendOneSignalPushNotification({
      title: 'PinIn Alert 🔔',
      message: 'Test notification from PinIn! Mobile push alerts are working perfectly.',
      url: window.location.href,
    });
    setIsSendingTest(false);
    if (result.success) {
      setPushStatusMessage('✅ Push notification sent to your device!');
    } else {
      setPushStatusMessage(`Push note: ${result.error || 'Check OneSignal setup'}`);
    }
  ...
  return (
    <div className="fixed inset-0 z-50 bg-gray-50 flex flex-col h-[100dvh] max-h-[100dvh] overflow-hidden font-sans select-none">
      ...
      <main className="flex-1 w-full max-w-md md:max-w-3xl lg:max-w-4xl mx-auto overflow-y-auto px-4 md:px-6 lg:px-8 pt-4 pb-6 space-y-3 text-left">
        {/* Title Bar */}
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-3.5 shadow-xs space-y-2.5">
          ...
          {pushStatusMessage && (
            <div className="p-2 rounded-xl bg-blue-50 text-[11px] font-medium text-[#0052FF] border border-blue-200">
              {pushStatusMessage}
            </div>
          )}
          ...
          <div className="flex items-center gap-2 pt-1">
            {!hasPushPermission ? (
              <button
                ...
                className="flex-1 py-2 px-3 bg-[#0052FF] hover:bg-blue-700 active:scale-95 text-white font-extrabold text-xs rounded-xl shadow-xs transition-all flex items-center justify-center gap-1.5 cursor-pointer"
              >
              ...
              <button
                ...
                className="flex-1 py-2 px-3 bg-blue-50 hover:bg-blue-100 text-[#0052FF] font-bold text-xs rounded-xl border border-blue-200 transition-colors flex items-center justify-center gap-1.5 cursor-pointer disabled:opacity-50"
              >
